...
const Portfolio = () => {
  ...
  // Function to render moredocandfiles sections
  const renderDocumentGrid = () => {
    return (
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        {moredocandfiles.sections.map((section, index) => (
          <div key={index} className="bg-gray-800 bg-opacity-90 rounded-lg p-4">
            <h4 className="font-semibold mb-2">{section.subtitle}</h4>
            <a
              href={section.images[0].content}
              target="_blank"
              rel="noopener noreferrer"
  ...
  // Function to render the textx section
  const renderTextxSection = () => {
    return textx.section.map((section, index) => (
      <div
        key={index}
        className="bg-gray-800 bg-opacity-90 rounded-lg p-6 mb-8"
      >
    ...
    <div className="flex text-lg">
      ...
      <div className="pt-32">
        ...
        {subQ1.section.map((section, index) => (
          <div className="bg-gray-800 bg-opacity-20 rounded-lg p-6 pt-1 mb-8">
            <div
              key={section.title}
              ref={(ref) => (sectionRefs.current[section.title] = ref)}
              className="mb-8"
            ></div>
            <div className="bg-gray-800 bg-opacity-40 rounded-lg p-6 mb-8">
        ...
        {/* Render moredocandfiles sections */}
        {/* {textx.sections.map((section, index))} */}
        <div className="mb-8">
          {/* Render textx section */}
          {renderTextxSection()}
